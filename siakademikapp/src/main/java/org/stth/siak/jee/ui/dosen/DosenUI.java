...
public class DosenUI extends UI {
	...
	private void setDrawerVariant(Behaviour variant) {
		...
		NavigatorAppLayoutBuilder drawer = AppLayout.getDefaultBuilder(variant)
				...
				.withDefaultNavigationView(DosenProfil.class)
				.withDesign(AppBarDesign.MATERIAL)
				//.withNavigatorConsumer(navigator -> {/* Do someting with it */})
				.add(hl, HEADER)
				//.add("PROFIL", VaadinIcons.USER, badge, new DosenProfil())
		...
		new DosenMenuComponent(drawer);
		AppLayoutComponent alc = drawer.build();
		holder.addComponent(alc);
		getNavigator().setErrorView(DosenProfil.class);
...
//		});
	...
	@VaadinServletConfiguration(productionMode = true, ui = DosenUI.class)
